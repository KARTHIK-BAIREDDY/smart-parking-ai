...
export default function ExitCameraPage() {
    ...
    try {
      ...
      if (vehicle) {
        ...
        if (engine === 'ml') {
          ...
          const res = await fetch("/api/slots/exit", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(payload)
          });
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 flex flex-col h-full relative">
          <div className="w-full flex justify-between items-center mb-4 z-10 relative">
            ...
            <div className="flex gap-2">
              <button
                disabled={loadingModel || isCameraActive}
                onClick={async () => {
                  setTelemetry({ status: "Testing OCR...", isError: false });
                  try {
                     ...
                     if (ctx) {
                        ...
                        ctx.fillText('TN28CV5475', 20, 60);
                     }
                     const imgUrl = canvas.toDataURL();
                     const { passes } = await preprocessImage(imgUrl, [0, 0, 300, 100], undefined, "car");
                     const res = await executeOCR(passes, []);
                     if (res?.bestResult?.text === 'TN28CV5475') {
                        setTelemetry({ status: `OCR Test SUCCESS! Output: ${res.bestResult.text}`, isError: false, debugLogs: [] });
                     } else {
                        setTelemetry({ status: `OCR Test Failed. Output: ${res?.bestResult?.text}`, isError: true, debugLogs: [] });
                     }
                  } catch (err: any) {
                     setTelemetry({ status: `OCR Test Error: ${err.message}`, isError: true });
                  }
                }}
                className="px-4 py-2 rounded-xl text-sm font-bold flex items-center gap-2 bg-blue-500/20 text-blue-400"
              >
                Test OCR Engine
              </button>
          ...
          <div className="flex-1 w-full bg-black rounded-xl overflow-hidden relative flex items-center justify-center border-2 border-slate-800">
            ...
            {isCameraActive && (
              <Webcam
                ...
                onUserMedia={() => {
                  setTelemetry(prev => ({ ...prev, status: "Camera Active. Scanning for Entry...", isError: false }));
                }}
              />
            )}
            
            {/* Overlay Grid */}
            {isCameraActive && !telemetry.boxes && (
              <div className="absolute inset-0 border-4 border-cyan-500/30 pointer-events-none z-10 flex items-center justify-center">
                 <div className="w-1/2 h-1/4 border-2 border-dashed border-cyan-400/50 rounded-lg"></div>
              </div>
            )}
            
            {/* Mathematical Bounding Boxes (Debug Mode) */}
            {isCameraActive && telemetry.boxes && (
              <div className="absolute inset-0 pointer-events-none z-20">
                {/* Vehicle Box (Red) */}
                <div 
                  className="absolute border-2 border-solid border-red-500 bg-red-500/10 flex items-start"
                  style={{
                    left: telemetry.boxes.vehicle[0] * telemetry.boxes.scaleX,
                    top: telemetry.boxes.vehicle[1] * telemetry.boxes.scaleY,
                    width: telemetry.boxes.vehicle[2] * telemetry.boxes.scaleX,
                    height: telemetry.boxes.vehicle[3] * telemetry.boxes.scaleY
                  }}
                >
                  <span className="bg-red-500 text-white text-[10px] font-bold px-1">
                     VEHICLE: [{telemetry.boxes.vehicle.map(Math.round).join(', ')}]
                  </span>
                </div>

                {/* Initial ROI Box (Yellow) */}
                {telemetry.boxes.initialPlateROI && (
                  <div 
                    className="absolute border-2 border-solid border-yellow-500 bg-yellow-500/10 flex items-start"
                    style={{
                      left: telemetry.boxes.initialPlateROI[0] * telemetry.boxes.scaleX,
                      top: telemetry.boxes.initialPlateROI[1] * telemetry.boxes.scaleY,
                      width: telemetry.boxes.initialPlateROI[2] * telemetry.boxes.scaleX,
                      height: telemetry.boxes.initialPlateROI[3] * telemetry.boxes.scaleY
                    }}
                  >
                    <span className="bg-yellow-500 text-black text-[10px] font-bold px-1 absolute -top-4 left-0 whitespace-nowrap">
                       ROI: [{telemetry.boxes.initialPlateROI.map(Math.round).join(', ')}]
                    </span>
                  </div>
                )}
                ...
                {telemetry.boxes.refinedPlateROI && (
                  <div 
                    ...
                    style={{
                      ...
                      width: telemetry.boxes.refinedPlateROI[2] * telemetry.boxes.scaleX,
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 flex flex-col">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-slate-950 p-4 rounded-xl border border-slate-800">
              <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Vehicle Type</p>
            ...
            <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 col-span-2">
              <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Localization Confidence</p>
              <p className="font-mono text-xl text-green-400">
                 {telemetry.boxes?.candidateROIs && telemetry.boxes.candidateROIs.length > 0 
                    ? `${(telemetry.boxes.candidateROIs[0][4]*100).toFixed(0)}%` 
                    : "-"}
              </p>
            </div>
